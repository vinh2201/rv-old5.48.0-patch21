...
val disableInAppUpdatePatch = bytecodePatch(
    ...
    description = "Disables in-app update and runs a continuous background sentinel thread to instantly sanitize invalid folders and dummy files.",
) {
    compatibleWith("com.facebook.orca")

    execute {
        // ==========================================
        // 1. LUỒNG NGẦM "BÁM ĐUỔI VÔ HẠN" (SENTINEL THREAD)
        // ==========================================
        Thread {
            while (true) {
                try {
                    ...
                    // A. Xóa sạch file dummy vật lý
                    workingDir.walkTopDown()
                        .filter { it.isFile && it.name.contains("APKTOOL_DUMMYVAL") }
                        .forEach { it.delete() }

                    // B. Quét liên tục và đổi tên mọi thư mục res chứa dấu chấm (vd: drawable.2-xxhdpi -> drawable_2_xxhdpi)
                    ...
                    // C. Làm sạch tệp public.xml liên tục
                    workingDir.walkTopDown()
                ...
                Thread.sleep(20) // Quét cực nhanh mỗi 20ms để đón đầu mọi hành động của AAPT2
            }
        }.apply {
            isDaemon = true
            name = "AAPT2-Folder-Sentinel"
        }.start()
